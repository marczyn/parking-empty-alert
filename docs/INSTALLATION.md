...
This is the **detailed** installation guide.
For a 3-line install via pre-built Docker image see [QUICKSTART](QUICKSTART.md).
...
|---|---|---|
| **A) Pre-built AIO image** ([QUICKSTART](QUICKSTART.md)) | Most users — want it working fast | 15 minutes |
| **B) Git clone + setup.sh** (this guide) | Advanced — want to customize configs, contribute, run examples (multi-camera, LPR, etc.) | 45-60 min |

If you don't need to customize anything, use Path A. If you need multi-camera, LPR, or custom Frigate model, use Path B.
