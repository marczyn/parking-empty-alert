...
⚠️ **Lovelace YAML mode is active** (set by `lovelace: mode: yaml` in
`configuration.yaml`). This means the **Edit Dashboard** button in HA UI is
grayed out — all dashboard changes happen by editing
`config/homeassistant/ui-lovelace.yaml`.
