...
EOF
  ...
  echo "✓ .env already exists. Syncing secrets.yaml + checking for missing keys..."
  # Always re-sync secrets.yaml from .env to prevent drift after manual .env edits
  set -a
...
EOF
  chmod 600 config/homeassistant/secrets.yaml
  echo "  ✓ secrets.yaml re-synced from .env"
